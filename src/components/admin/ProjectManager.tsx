...
const ProjectManager = () => {
  ...
  const handleToggleActive = async (project: CustomProject) => {
    ...
    if (error) {
      ...
      toast.success(project.active ? 'Projeto desativado' : 'Projeto ativado');
      fetchProjects();
    }
  };
  ...
  const handleDuplicate = (project: CustomProject) => {
    setEditingProject({
      ...
      project_name: `${project.project_name} (Cópia)`,
    });
    setModalOpen(true);
  };
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                <TableCell colSpan={6} className="text-center py-12 text-muted-foreground">
                  Nenhum projeto criado ainda. Crie o primeiro!
                </TableCell>
              </TableRow>
            ) : (
              projects.map((p) => (
                <TableRow key={p.id} className="border-border/30">
                  ...
                  <TableCell>
                    <span className="font-mono-code">{clientCounts[p.id] || 0}</span>
                  </TableCell>
                  <TableCell>
