...
function buildPersonDescription(profile: Record<string, unknown>): string {
  ...
  if (p.body?.height_estimate && p.body.height_estimate !== "não visível") parts.push(`${p.body.height_estimate} stature`);
  if (p.face?.shape) { parts.push(`${p.face.shape} face shape`); if (p.face.shape_details) parts.push(p.face.shape_details); }
  ...
  if (p.eyes?.color) { parts.push(`${p.eyes.shape || ""} ${p.eyes.color} eyes`.trim()); if (p.eyes.spacing) parts.push(`${p.eyes.spacing} set`); if (p.eyes.brows) parts.push(`${p.eyes.brows}`); }
  if (p.nose?.profile) parts.push(`${p.nose.profile} nose with ${p.nose.nostril_width || "medium"} nostrils`);
  if (p.lips?.size) { parts.push(`${p.lips.size} lips`); if (p.lips.upper) parts.push(`${p.lips.upper} upper lip`); if (p.lips.cupids_bow !== "ausente") parts.push(`${p.lips.cupids_bow} cupid's bow`); if (p.lips.natural_color) parts.push(`natural ${p.lips.natural_color} lip color`); }
  if (p.unique_features?.features?.length > 0) parts.push(`Distinctive features: ${p.unique_features.features.join(", ")}`);
  if (p.hair?.color_base) {
    let hairDesc = `${p.hair.length || "medium"} ${p.hair.texture || ""} ${p.hair.color_base} hair`.trim();
    if (p.hair.highlights && p.hair.highlights !== "sem reflexos") hairDesc += ` with ${p.hair.highlights}`;
    if (p.hair.volume) hairDesc += `, ${p.hair.volume} volume`;
    parts.push(hairDesc);
  }
  if (p.photography_notes?.emphasize?.length > 0) parts.push(`Key identifying features: ${p.photography_notes.emphasize.join(", ")}`);
  return parts.join(". ") + ".";
}
...
async function generateInBackground(
  ...
  quantity: number,
  creditCost: number,
  creditsData: { id: string; credits_available: number; credits_used: number } | null,
) {
  ...
  try {
    ...
    if (profileError || !profileData) {
      await supabase.from("photo_shoots").update({ status: "failed", error_message: "Perfil não encontrado" }).eq("id", shootId);
      return;
    }

    const profile = profileData.profile_document as Record<string, unknown>;

    // Download reference photos as data URLs (limit to 3 to reduce CPU)
    const refPaths = (profileData.reference_photo_paths as string[] || []).slice(0, 3);
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      try {
        ...
        if (!imageDataUrl) { console.error(`No image data for photo ${i + 1}`); continue; }

        const base64Match = imageDataUrl.match(/^data:image\/\w+;base64,(.+)$/);
        if (!base64Match) { console.error(`Invalid image data URL for photo ${i + 1}`); continue; }

        const imageBytes = Uint8Array.from(atob(base64Match[1]), c => c.charCodeAt(0));
        const filePath = `${userId}/shoots/${shootId}/${i + 1}.png`;
    ...
    for (const path of generatedPhotoPaths) {
      const { data } = await supabase.storage.from("studio-generated-photos").createSignedUrl(path, 60 * 60 * 24 * 7);
      if (data?.signedUrl) signedPhotoUrls.push(data.signedUrl);
    }

    await supabase.from("photo_shoots").update({
      status: finalStatus,
    ...
    if (generatedPhotoPaths.length > 0 && creditsData?.id) {
      await supabase.from("studio_credits").update({
    ...
    await supabase.from("photo_shoots").update({
      status: "failed",
      error_message: (error as Error).message || "Erro interno",
    }).eq("id", shootId);
  }
}
  ...
  try {
    ...
    EdgeRuntime.waitUntil(
      generateInBackground(
        ...
        quantity,
        creditCost,
        creditsData ? { id: creditsData.id, credits_available: available, credits_used: creditsData.credits_used } : null,
      )
    );

    // Return immediately with shoot ID for polling
    return new Response(
      JSON.stringify({ shoot_id: shoot.id, status: "processing" }),
