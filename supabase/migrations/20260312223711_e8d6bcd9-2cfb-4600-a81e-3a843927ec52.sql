...
BEGIN
  ...
  -- Rule 4: 30-day cooldown from last session
  SELECT MAX(cs.scheduled_date) INTO v_last_session_date
  FROM capture_sessions cs
  WHERE cs.user_project_id = NEW.user_project_id
    AND cs.status IN ('scheduled', 'confirmed', 'completed')
    AND cs.id IS DISTINCT FROM NEW.id;

  IF v_last_session_date IS NOT NULL AND (CURRENT_DATE - v_last_session_date) < 30 THEN
    RAISE EXCEPTION 'É necessário aguardar 30 dias desde o último agendamento (último: %)', v_last_session_date;
  END IF;
